...
def download_xls():
    ...
    if minus < 7:
        startdate = dt.now() - datetime.timedelta(days=minus+6)
        ...
        startdate = dt.now() - datetime.timedelta(days=6)
        smonth = startdate.month
        sday = startdate.day
        enddate = dt.now()
        emonth = enddate.month
    ...
    time.sleep(3)
    driver.quit()
    return f'{smonth}.{sday}~{emonth}.{eday}'
